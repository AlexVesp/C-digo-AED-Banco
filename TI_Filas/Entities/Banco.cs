...
namespace TI_Filas.Entities
{
    static class Banco
    {
        ...
        */
        public static void inicializaBanco(List<Cliente> pessoasCarregadas)
        {
            ...
            do
            {
                ...
                {
                    int AtendimentoFila1 = atendimento(fila1, 1);
                    if (AtendimentoFila1 > -1)
                    {
                        TempoTotalEspera += turno - AtendimentoFila1;
                        PessoaMaiorTempo = PessoaMaiorTempo < turno - AtendimentoFila1 ? turno - AtendimentoFila1 : PessoaMaiorTempo;
                        PessoasAtendidas++; //atendimento (fila + um inteiro só pra sinalizar na mensagem)
                    }

                    int AtendimentoFila2 = atendimento(fila2, 2);
                    if (AtendimentoFila2 > -1)
                    {
                        TempoTotalEspera += turno - AtendimentoFila2;
                        PessoaMaiorTempo = PessoaMaiorTempo < turno - AtendimentoFila2 ? turno - AtendimentoFila2 : PessoaMaiorTempo;
                        PessoasAtendidas++; //atendimento (fila + um inteiro só pra sinalizar na mensagem)
                    }

                    int AtendimentoFila3 = atendimento(fila3, 3);
                    if (AtendimentoFila3 > -1)
                    {
                        TempoTotalEspera += turno - AtendimentoFila3;
                        PessoaMaiorTempo = PessoaMaiorTempo < turno - AtendimentoFila3 ? turno - AtendimentoFila3 : PessoaMaiorTempo;
                        PessoasAtendidas++; //atendimento (fila + um inteiro só pra sinalizar na mensagem)
                    }

                    int AtendimentoFila4 = atendimento(fila4, 4);
                    if (AtendimentoFila4 > -1)
                    {
                        TempoTotalEspera += turno - AtendimentoFila4;
                        PessoaMaiorTempo = PessoaMaiorTempo < turno - AtendimentoFila4 ? turno - AtendimentoFila4 : PessoaMaiorTempo;
                        PessoasAtendidas++; //atendimento (fila + um inteiro só pra sinalizar na mensagem)
                    }

                    int AtendimentoFila5 = atendimento(fila5, 5);
                    if (AtendimentoFila5 > -1)
                    {
                        TempoTotalEspera += turno - AtendimentoFila5;
                        PessoaMaiorTempo = PessoaMaiorTempo < turno - AtendimentoFila5 ? turno - AtendimentoFila5 : PessoaMaiorTempo;
                        PessoasAtendidas++; //atendimento (fila + um inteiro só pra sinalizar na mensagem)
                    }
            ...
            Console.WriteLine("Tempo De execução total: " + turno);
            Console.WriteLine("Maior Tempo de Espera: " + PessoaMaiorTempo);
            Console.WriteLine("Tempo Médio de Espera: " + (TempoTotalEspera / PessoasAtendidas));
            Console.WriteLine("Pessoas atendidas: " + PessoasAtendidas);
        }
        ...
        */
        public static int atendimento(List<Cliente> fila, int caixa)
        {
            int atendido = -1;
            if (fila.Count != 0)
            { //esse if valida se a fila está vazia... se esiver é printado no console caixa vazio.
                ...
                atendido = fila[0].turnoDeChegada;
